...
#
# Usage:   setup-worktree.sh <TICKET_ID> <BRANCH> [SPEED]
# Example: setup-worktree.sh HEL-55 feature/panel-dup/HEL-55 fast
#
#   SPEED  optional; fast|default|slow (defaults to "default" when omitted,
#          exactly like resolve-speed.sh itself does). Resolved here — once,
#          this is the run's only resolution of it — via
#          `scripts/concertino/resolve-speed.sh "$SPEED" "$HARNESS"`, passing
#          the harness this script already computed below rather than
#          re-detecting it a second time. The resulting budgets/models/flags
#          are folded into this script's own run.start emission and READY
#          contract (see both below) — this is the actual, ground-truth
#          run.start emission site (delivery-speed-presets design.md
#          Decision 3a), so a run's resolved rigour is recorded truthfully
#          in the same event that already defines the run's identity.
#
...
TICKET_ID="${1:?usage: setup-worktree.sh <TICKET_ID> <BRANCH> [SPEED]}"
BRANCH="${2:?usage: setup-worktree.sh <TICKET_ID> <BRANCH> [SPEED]}"
SPEED="${3:-default}"
...
# Resolve the speed against the harness just computed above — passed
# explicitly rather than left for resolve-speed.sh to re-detect, so the two
# scripts' independent detection logic can never disagree with itself
# within one run (design.md Decision 3a). This is the run's ONE authoritative
# resolution: the orchestrator does not call resolve-speed.sh again itself,
# it just parses the READY lines this script prints below.
#
...
RESOLVED_SPEED_JSON="$("${SCRIPT_DIR}/resolve-speed.sh" "$SPEED" "$HARNESS" 2>&1)" || {
  echo "FAIL speed resolution failed for speed=${SPEED} harness=${HARNESS}: ${RESOLVED_SPEED_JSON}" >&2
  exit 1
}
