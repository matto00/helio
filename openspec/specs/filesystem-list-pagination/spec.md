# filesystem-list-pagination Specification

## Purpose
Defines the cursor-based paginated list API (`ListPage` model and `list(prefix, cursor, pageSize)` method) for `FileSystem`, enabling both `GcsFileSystem` and `LocalFileSystem` to return bounded pages of object names instead of materialising the full result set in memory.
## Requirements
