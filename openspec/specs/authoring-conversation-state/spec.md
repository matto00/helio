# authoring-conversation-state Specification

## Purpose
Server-persisted, owner-scoped, multi-turn refinement of an NL-authored dashboard proposal before
it's ever applied — the same proposal revised turn over turn (deterministic history bounding, a
per-conversation token ceiling, and a reload-survival hydration route), never a second apply path.
## Requirements
