# dashboard-contents-replace Specification

## Purpose
Lets a caller atomically replace all of an existing dashboard's panels in a single transaction, so an agentic
rebuild (e.g. `helio-news`) never leaves the live dashboard half-empty on a mid-rebuild failure.
## Requirements
...
created and the prior panel set remains exactly as it was.
...
- **WHEN** the payload's panel at index 2 has an invalid `type` or is missing a required `dataTypeId`
...
- **WHEN** a supplied panel's `dataTypeId` resolves to a companion (non-pipeline) DataType
