...
required config fields), exposed via `GET /api/connector-types` and an MCP `list_connector_types`
tool, so an agent or the frontend "Add source" toggle can discover what connectors exist and what
each needs.
