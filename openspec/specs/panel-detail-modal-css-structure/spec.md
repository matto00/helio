# panel-detail-modal-css-structure Specification

## Purpose
Keeps the panel-detail modal stylesheets within the design system and file-size budget: spacing uses exact DESIGN.md `--space-*` tokens, no split file exceeds the ~400-line soft budget, and the mobile ≥44px tap-target CSS-locks are preserved so the modal stays behavior-preserving and pixel-stable.
## Requirements
