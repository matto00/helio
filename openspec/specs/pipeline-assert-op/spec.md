# pipeline-assert-op Specification

## Purpose
Let a pipeline declare data-quality expectations about its own output as a pass-through `assert` step —
the rule model, persistence, and schema-inference scaffolding rule evaluation (419-B) builds on.
