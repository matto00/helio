# settings-agent-memory-ui Specification

## Purpose
The agent-memory management surface on the `/settings` page: view every stored memory entry
(kind/content/last-used), delete individual entries, and clear all, using this codebase's
established inline confirm/cancel pattern (never `window.confirm`) with visible error feedback
and toast parity matching every other destructive delete action in the app.
## Requirements
