...
## Purpose
Provides one shared, accessible icon-only button primitive (`shared/ui/IconButton`) so every icon-only
control in the app carries a required accessible name and a visible tooltip by default, instead of each
feature hand-rolling its own inconsistent recipe.
## Requirements
