# openspec-archival-hygiene Specification

## Purpose

Defines when the OpenSpec hygiene check reports an unarchived change whose tasks are fully checked as overdue for
archival, and when it instead exempts that change as still in flight — so the check fires on genuine archival
