# pipeline-assert-fail-policy Specification

## Purpose
Block a pipeline run's DataType update when an error-severity assertion fails, so bad data never
reaches bound panels or metrics, while a warn-severity failure is recorded without blocking the run.
## Requirements
