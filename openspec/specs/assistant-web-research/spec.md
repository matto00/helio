# assistant-web-research Specification

## Purpose
Grounds the top-level workspace assistant's REST-source proposals in real, current external
documentation via Anthropic's server-side `web_search` tool, so base URL/auth/field-shape claims are
checked against reality instead of relying solely on the model's training-data recall.
## Requirements
