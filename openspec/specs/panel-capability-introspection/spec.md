# panel-capability-introspection Specification

## Purpose
Expose, server-side, which panel kinds a DataType can structurally bind to and what field-mapping slots
each needs, so external agents (e.g. helio-news) build their offers menu from Helio's own ground truth
instead of hand-deriving and duplicating its binding rules.
## Requirements
