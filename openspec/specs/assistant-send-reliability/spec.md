# assistant-send-reliability Specification

## Purpose
Client-side send reliability for the workspace assistant: one stable idempotency key per logical
message and post-error reconciliation against the server, so the displayed state after a send
error reflects whether the message actually landed rather than always implying total failure.
## Requirements
