# mcp-assertion-results-grounding Specification

## Purpose
Let an MCP agent read each pipeline's latest-run assertion trustworthiness signal via
`get_workspace_context`, so it can reason about whether a dashboard's data can be trusted.
## Requirements
