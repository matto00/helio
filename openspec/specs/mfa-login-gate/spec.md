...
## Purpose
Backend session-establishment gate: MFA-enabled primary auth (password or Google OAuth) yields a short-lived challenge instead of a session; a public verify endpoint exchanges the challenge plus a TOTP or backup code for the session.
## Requirements
