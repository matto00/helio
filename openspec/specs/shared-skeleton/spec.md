# shared-skeleton Specification

## Purpose
Shared `Skeleton` primitive (`frontend/src/shared/ui/Skeleton.tsx`) providing the block/line/circle
placeholder shapes every loading skeleton in the app is composed from, with token-only styling, a
neutral shimmer that is genuinely disabled under `prefers-reduced-motion`, and decorative (`aria-hidden`)
semantics so a loading region announces itself once rather than once per placeholder.
