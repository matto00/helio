# agent-memory-retention Specification

## Purpose
Age-based data-minimization retention for `AgentMemoryEntry`, enforced on access (no scheduler):
entries past a documented, configurable retention window are excluded from reads and actually
pruned, independent of the per-user eviction cap (420-B) and never extended by usage.
## Requirements
