...
reached only once before the first fetch, SHALL NOT use that widened condition, since it would render a
skeleton indefinitely. Two such surfaces exist and SHALL be treated as exceptions:
...
dashboard — the initial-load condition SHALL remain keyed on an in-flight fetch only, accepting a single
pre-dispatch frame. The panel list SHALL NOT render a skeleton in either state where no fetch is pending
or coming: when no dashboard is selected, and when the selected dashboard's panels have been invalidated
or emptied without a refetch being scheduled. In the first of those it renders its existing empty state
and create-dashboard action. In the second it currently renders nothing, because its empty-state branch
is conditioned on a resolved status that the invalidation has already replaced; that missing empty state
is a pre-existing gap in the empty-state capability and is NOT closed by this capability, which requires
only that no misleading loading state be shown there.
