...
The panel create form MUST present a type selector control offering all available panel types (`metric`, `chart`, `text`, `table`) with `metric` pre-selected.

#### Scenario: Type selector appears when create form opens
- **WHEN** the user opens the panel create form
- **THEN** a type selector is visible with all four type options
...
- **GIVEN** the user selected `text` in the type selector
