# assistant-conversation-persistence Specification

## Purpose
Persistent, listable assistant conversations — slim Postgres metadata (title, pinned, timestamps)
plus a transcript body stored via the existing local/GCS uploads-backend abstraction — so HEL-659's
top-level assistant can create, append to, list (pinned-first, most-recent-first), and pin/unpin
conversations, each strictly scoped to its owner via RLS.
## Requirements
