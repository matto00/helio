# pipeline-unpivot-op Specification

## Purpose
The `unpivot` pipeline op reshapes wide rows into long rows — collapsing many value columns into a
`variable`/`value` column pair, repeating the id columns per row — with a fully statically-knowable
output schema (no data sampling) to normalize wide sources before aggregation.
## Requirements
