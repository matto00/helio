# proposal-apply-dashboard-refresh Specification

## Purpose
Ensure that applying a dashboard proposal from the Proposal Review UI immediately updates the Redux dashboards list
with the created dashboard, so the sidebar never shows a stale list or empty state.
## Requirements
