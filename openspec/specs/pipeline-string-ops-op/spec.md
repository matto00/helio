# pipeline-string-ops-op Specification

## Purpose
The `stringops` pipeline op performs per-value string cleaning (trim, case conversion,
split-and-take, regex extraction, field concatenation) and writes the result to a derived string
column, distinct from the row-exploding `splittext` op.
## Requirements
