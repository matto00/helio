...
## Purpose
Lets users upload an image directly (rather than only linking an external URL) via a
FileSystem-backed endpoint, storing it under an owner-scoped `image_uploads` record and serving it
back through a stable, unauthenticated capability URL usable by a plain `<img src>`.
## Requirements
