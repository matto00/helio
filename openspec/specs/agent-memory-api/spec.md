...
## Purpose
`GET/POST /api/agent/memory`, `DELETE /api/agent/memory/:id`, and `DELETE /api/agent/memory` on
the authenticated route tree, exposing create/list/delete/clear operations over a user's
free-form agent memory — the 420-B half of Agent Memory & Preferences (HEL-420).
## Requirements
