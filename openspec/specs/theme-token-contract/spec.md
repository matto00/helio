# theme-token-contract Specification

## Purpose
Keeps a theme's stylesheet honest about which token values actually render, so that a value declared per theme cannot be silently overridden at runtime while still appearing to a reader as deliberate per-theme tuning.
