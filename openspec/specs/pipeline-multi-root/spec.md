# pipeline-multi-root Specification

## Purpose
A pipeline owns an ordered, non-empty set of source roots, each binding one DataSource. This capability covers root identity and ordering, root lifecycle (add, remove, position compaction), per-root access control, how a step and a root-bound Output name the root they belong to, and how a root is identified on the wire.. Update Purpose after archive.
