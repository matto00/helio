# csv-url-ingestion Specification

## Purpose
Defines CSV data sources created from an HTTPS URL: the https-only and internal-address rejection rules, and re-fetching that URL on both manual refresh and scheduled pipeline runs so a CSV-backed dashboard can stay current without re-upload.. Update Purpose after archive.
