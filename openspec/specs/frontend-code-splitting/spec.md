...
## Purpose
Heavy, non-critical-path frontend surfaces (chart rendering, markdown rendering, the Proposal Review route) load via dynamic `import()` rather than the initial entry chunk, behind a shared `Spinner`-based `Suspense` fallback, with a bundle-visualizer report available on demand to track chunk composition.
## Requirements
