# empty-state-cta-pattern Specification

## Purpose
The cross-section standard for empty states in the primary workspace sections: main-variant hero on
page surfaces, one primary call to action wired to that section's real create flow, and a
filtered-to-zero state that reads as a different situation from nothing-created-yet.
## Requirements
