# pipeline-schedule-persistence Specification

## Purpose
Durable, owner-scoped (via parent pipeline) storage of per-pipeline cron/interval schedules — the
`PipelineSchedule` domain model, Flyway schema, RLS, and repository access patterns that the
scheduler runtime (a sibling ticket) will poll to fire pipeline runs.
## Requirements
