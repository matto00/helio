...
- **THEN** the system returns `201 Created` with `{ token, expiresAt, user: { id, email, displayName, createdAt } }`
...
- **THEN** the system returns `200 OK` with `{ token, expiresAt, user: { id, email, displayName, createdAt } }`
