# orbit-logo-mark Specification

## Purpose
Defines requirements for the Orbit SVG logo mark that replaces the dot placeholder in the Helio command bar and sidebar, including its geometry, theming, and static asset availability.
...
- **WHEN** the Orbit mark SVG renders
- **THEN** a `drop-shadow` filter using the accent color SHALL be applied to create the glow effect
