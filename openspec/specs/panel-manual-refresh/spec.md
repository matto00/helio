# panel-manual-refresh Specification

## Purpose
Gives every output-bound panel a keyboard-accessible Refresh control that immediately re-fetches its bound Output, with a single shared in-flight guard so the manual, interval-poll, and pipeline-run SSE fan-out triggers never race a duplicate concurrent fetch.
