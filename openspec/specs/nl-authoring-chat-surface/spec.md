# nl-authoring-chat-surface Specification

## Purpose
The in-app entry point for natural-language dashboard authoring — a chat drawer where a user states
a goal, watches a streamed response, and hands the resulting validated proposal off to the existing
Proposal Review UI for explicit accept/apply, without any changes to that review/apply path.
## Requirements
