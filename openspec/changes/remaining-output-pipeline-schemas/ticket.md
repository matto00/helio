...
1. `inferredSchema` on `DataSourceResponse` — **ALREADY SHIPPED** (field exposed on GET/POST list, single-resource, and create responses). Only `schemas/sources/data-source.schema.json` is missing — **still needed**.
