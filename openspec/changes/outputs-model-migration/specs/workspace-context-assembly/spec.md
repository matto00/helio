...
`pathPrefix("workspace")`, returning a single JSON snapshot of the caller's data sources, Output/nodes,
...
- **WHEN** an authenticated user with at least one data source, Output/node, pipeline, and dashboard
...
- **WHEN** an authenticated user with no data sources, Output/nodes, pipelines, or dashboards calls
...
- **GIVEN** an authenticated user with at least one Output/node carrying sample rows and column
  statistics
...
- **GIVEN** user A owns a data source, Output/node, pipeline, and dashboard, and user B owns a distinct set
...
Each entry in `dataTypes` SHALL set `pipelineOutput = true` if and only if the Output/node's `sourceId` is
absent (a pipeline-produced, panel-bindable Output/node); an Output/node with a present `sourceId` (a
source-companion Output/node) SHALL set `pipelineOutput = false`.

#### Scenario: Source-companion DataType is flagged non-bindable
- **GIVEN** an Output/node created alongside a CSV data source (`sourceId` set)
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
...
- **GIVEN** an Output/node produced by a pipeline run (`sourceId` absent)
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
...
- **GIVEN** a workspace with two pipelines, one of which fails to analyze (e.g. its source Output/node was
  deleted)
...
Each `dataTypes[]` entry SHALL carry a `sampleRows` field: up to 5 rows read from the Output/node's latest
pipeline-run snapshot, each row limited to the first 40 of the Output/node's declared *Structured-category*
...
Output/node with no run snapshot, or a source-companion Output/node (never written to the snapshot), SHALL
...
- **GIVEN** a source-companion Output/node (`sourceId` present)
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
- **THEN** the corresponding `dataTypes[].sampleRows` is `[]`

### Requirement: Sample-row size caps enforced by construction
Sample rows SHALL be bounded independently of workspace or Output/node size: at most 5 rows (bounded at the
...
- **GIVEN** an Output/node snapshot row containing a string value longer than 200 characters in a
...
- **GIVEN** an Output/node with more than 40 declared Structured-category fields
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
- **THEN** each entry in the corresponding `dataTypes[].sampleRows` contains keys for at most the first
  40 of that Output/node's declared Structured-category fields, in field order
...
Sample rows SHALL only be readable by the Output/node's owner, via the same ownership check
...
entry per Structured-category column of that Output/node. Each entry SHALL report `nullRate` (fraction of
...
`sampleRows` for that Output/node — no additional database query, and no query without a `LIMIT`. The shared
fetch's row bound SHALL be a fixed, documented constant, independent of the Output/node's true row count.
...
- **THEN** the response is `200` and `columnStats` is present, computed only over the bounded row window,
  not the Output/node's full row count
...
`columnStats` computation SHALL be bounded independently of the Output/node's data: the underlying row fetch
...
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
- **THEN** `dataTypes[].columnStats` contains entries for at most the first 40 of that Output/node's declared
...
`distinctCountCapped: false`, `exampleValues: []`, and no `min`/`max`/`mean`. A Output/node with no run
...
- **GIVEN** a pipeline-Output whose pipeline has never run successfully
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
- **THEN** `dataTypes[].columnStats` contains an entry for each of that Output/node's Structured-category
...
- **WHEN** `GET /api/workspace/context` is called twice in succession by that Output/node's owner
...
The response SHALL carry a top-level `joinHints` array: cross-Output/node pairs of `identifier`-role columns
(HEL-374, "Deterministic column semantic role") from the caller's own pipeline-Outputs, whose
normalized column name and declared-type bucket match, each reporting `leftOutput/nodeId`, `leftColumn`,
`rightOutput/nodeId`, `rightColumn`, and a `confidence` score in `[0.5, 1.0]`. Every hint SHALL be labelled
advisory/inferred; `joinHints` SHALL NOT be used to alter any Output/node's declared schema. The candidate
...
- **THEN** `joinHints` contains one entry with `leftColumn: "customer_id"` and `rightColumn:
  "customer_id"` referencing the two Output/nodes' ids, and `confidence` greater than `0.5`
...
- **WHEN** user B calls `GET /api/workspace/context`
- **THEN** `joinHints` contains no entry referencing user A's Output/node
...
- **WHEN** `GET /api/workspace/context` is called by that Output/node's owner
- **THEN** at most 40 of that Output/node's columns are considered as join-hint candidates
...
omitted): first `sampleRows` row count (uniformly across all Output/nodes), then
...
- **WHEN** `GET /api/workspace/context` is called by that workspace's owner
- **THEN** the response is `200`, every Output/node's `sampleRows` and `columnStats[*].exampleValues`
...
- **GIVEN** a workspace whose assembled response exceeds the effective budget, and which fits the
  budget once every Output/node's `sampleRows` is reduced to fewer than its natural count while every
  column's `exampleValues` remains at its natural size
- **WHEN** `GET /api/workspace/context` is called by that workspace's owner
- **THEN** the response is `200`, every Output/node's `sampleRows` array has fewer entries than its
  natural count (uniformly, the same cap applied to every Output/node), and every column's
...
- **GIVEN** a workspace whose assembled response still exceeds the effective budget even with every
  Output/node's `sampleRows` reduced to `[]`
- **WHEN** `GET /api/workspace/context` is called by that workspace's owner
- **THEN** the response is `200`, every Output/node's `sampleRows` is `[]`, and every column's
...
- **GIVEN** a workspace whose assembled response still exceeds the effective budget even with every
  Output/node's `sampleRows` reduced to `[]` and every column's `exampleValues` reduced to `[]`
- **WHEN** `GET /api/workspace/context` is called by that workspace's owner
- **THEN** the response is `200`, every Output/node's `sampleRows` and every column's `exampleValues`
...
- **GIVEN** a workspace with more than 200 Output/nodes
- **WHEN** the owner calls `GET /api/workspace/context`
...
- **GIVEN** a workspace with fewer than 200 Output/nodes, data sources, and dashboards
- **WHEN** the owner calls `GET /api/workspace/context`
...
- **GIVEN** a workspace with a small number of Output/nodes, each with a small number of sample rows
  and columns
