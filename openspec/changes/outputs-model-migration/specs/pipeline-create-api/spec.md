...
`name` (string, required, non-empty), `sourceDataSourceId` (string, required). The endpoint SHALL
create a new pipeline row referencing the given `sourceDataSourceId`, without minting any
DataType, and return a `201 Created` response with the created pipeline summary (`id`, `name`,
`sourceDataSourceName`, `lastRunStatus`, `lastRunAt`). `outputDataTypeName` is no longer accepted
or returned; the single-call surface that also creates Outputs at creation time lands in P1.3.

#### Scenario: Successful pipeline creation returns 201 with summary
- **WHEN** `POST /api/pipelines` is called with valid `{ name, sourceDataSourceId }`
- **THEN** the response is `201 Created` with a JSON body containing the new pipeline's `id`,
  `name`, `sourceDataSourceName` (from the referenced data source), `lastRunStatus: null`, and
  `lastRunAt: null`, and no `outputDataTypeName` field
