...
## 8. Fold-in addendum (post-delivery, before Phase 4 cleanup)

- [x] 8.1 New regression test: a hop-cap-exhausted turn with 2+ dangling `tool_use` blocks in the
      same assistant turn (not just one); assert the next `converse` call succeeds and every
      dangling block gets its own paired synthetic `isError` `tool_result` in the outbound request
- [x] 8.2 Confirm no production code change is needed (the `seedHistory` fix already handles this
      by construction) — if it turns out one IS needed, that's new information the test surfaced,
      fix it
- [x] 8.3 `sbt test` clean
