...
## Cycle 1 (delivered, PR #327 — CONFIRMed/PASSed)
...
## Cycle 2 (this cycle — fold-in, post-delivery follow-up A, tasks.md section 7)

- `backend/src/test/scala/com/helio/services/DashboardAuthoringServiceSpec.scala` — added 6 new
  cases driving `DashboardAuthoringService.mapClaudeError`'s three branches end-to-end (previously
  only unit-tested implicitly via the repair-round-trip cases, never via the error-mapping paths
  themselves):
  - `author`: `GuardrailExceeded` (tiny `ClaudeConfig.maxInputTokens`) → `422`; `ApiError` → `502`;
    `TransportFailure` → `502` (tasks 7.1/7.2).
  - `authorStreaming`: the same three cases mirrored, asserting the terminal
    `AuthoringStreamEvent.Error` carries the SAME mapped message the buffered path would produce for
    an identical failure — compared against the buffered path's actual runtime message, not a
    hand-duplicated format string (task 7.3).
  - No source (`main/`) changes this cycle — `mapClaudeError`'s implementation (D8) was already
    correct from cycle 1; this cycle only closes the missing test coverage `spec.md`'s new
