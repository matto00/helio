...
payload, persists the specified fields, and returns HTTP 200 OK with the updated preferences object.
The authenticated user's identity is derived from the session token — no `:id` path parameter is required.
