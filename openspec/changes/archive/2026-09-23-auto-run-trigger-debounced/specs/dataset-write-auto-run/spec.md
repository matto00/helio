...
not `autoRunnable` at the time eligibility is evaluated, and SHALL make the verdict's denial
reason(s) available (at minimum, logged) rather than discarding them silently.
...
- **THEN** no run is scheduled or submitted for that pipeline as a result of the write, and the
  denial reason is recorded rather than silently dropped
