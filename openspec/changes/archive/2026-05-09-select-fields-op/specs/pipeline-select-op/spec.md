...
checklist of available column names derived from the previous step's (or the source's) last run
result. Each checkbox SHALL correspond to one column name. Checked columns are included in `fields`;
unchecked columns are excluded. If no run result is available, the UI SHALL display a prompt
indicating the user must run the pipeline to preview available fields.

#### Scenario: Checklist shows columns from last run result
- **WHEN** a select step card is expanded and a prior run result with columns `["a", "b", "c"]` exists
- **THEN** the step-card body shows three checkboxes labelled `a`, `b`, and `c`

#### Scenario: No run result shows prompt
- **WHEN** a select step card is expanded and no run result exists
- **THEN** the step-card body shows a message prompting the user to run the pipeline first
