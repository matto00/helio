...
<!-- In-repo precedent: `pipeline-lookup-op` and `pipeline-union-op` already require a run-time failure naming
     the step when a reference id holds "the tolerant-decode default of an empty string". This capability
     generalises that established shape to every step kind's required values. -->
