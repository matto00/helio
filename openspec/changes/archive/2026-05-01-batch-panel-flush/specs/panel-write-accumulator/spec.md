...
> **Scope note — panel.type is immutable post-creation.**
> `panel.type` (metric / chart / text / table) is set when a panel is created via `PanelList.tsx`
> and cannot be changed through any UI after creation. Type accumulation is therefore **not in
> scope** for this change. The `chartType` field inside `appearance.chart` (bar / line / pie /
> scatter) is part of appearance and IS accumulated via the appearance path.
