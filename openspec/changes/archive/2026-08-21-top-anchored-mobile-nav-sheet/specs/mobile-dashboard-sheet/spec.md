...
**Reason**: The sheet is no longer bottom-anchored, and the requirement's "SHALL contain no
create/rename/delete/duplicate affordances" prohibition is deliberately narrowed by this change to permit
exactly one section-appropriate create action. Keeping the name would leave a requirement titled "Bottom
sheet…" sitting beside one titled "…descends from its trigger" in the same capability.

**Migration**: Replaced by "Top-anchored item picker" (dashboard listing, selection, active indication, and
the surviving no-CRUD prohibitions) together with "Section-appropriate create action in the sheet" (the
single permitted create affordance). No behaviour is dropped: every clause of the removed requirement is
carried by one of the two replacements, except the create prohibition, which this change intentionally
reverses.
