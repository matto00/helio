...
**Reason**: Loading is now rendered as a shape-matched skeleton by each consuming surface, so a single
one-line text block is no longer an acceptable loading treatment for a data-backed list (DESIGN.md §7 —
"never a flash of empty content"). Keeping the branch would leave dead code in a shared primitive after
all three consumers stopped routing loading through it.

**Migration**: Consumers render skeleton placeholders from the shared `Skeleton` primitive for their
initial-load state, and continue to use `StatusMessage` for the failed state only. `"loading"` is removed
from the component's `status` prop type, so any remaining call site passing it becomes a compile error
