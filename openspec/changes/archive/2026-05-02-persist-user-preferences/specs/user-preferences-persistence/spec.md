...
### Requirement: user_preferences JSONB column on users table
The database SHALL have a nullable `preferences` JSONB column on the `users` table to store per-user
global preferences (e.g. `accent_color`).

#### Scenario: Migration adds preferences column
- **WHEN** Flyway migration V18 runs on an existing database
- **THEN** the `users` table gains a nullable `preferences` JSONB column
...
- `getPreferences(userId): Future[UserPreferencesData]` — returns `accentColor` (from JSONB) and
  a map of `dashboardId → zoomLevel` (from `user_dashboard_zoom`).
- `upsertGlobalPrefs(userId, accentColor): Future[Unit]` — updates the JSONB column on `users`.
...
- **THEN** the `users.preferences` JSONB is updated to contain `{ "accentColor": "#3b82f6" }`
