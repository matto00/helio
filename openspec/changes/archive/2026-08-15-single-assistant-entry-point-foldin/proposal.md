...
- **Fold-in addendum (post-delivery, before Phase 4 cleanup):** delete the now-dead
  `AuthoringGoalRequest`/`AuthoringResult` interfaces from `authoring.ts` and their re-export from
  `authoringService.ts` — a direct, zero-risk consequence of this ticket's own earlier deletion,
  triaged fold-in rather than filed standalone (see ticket.md Context/Notes).
...
- `frontend/src/features/dashboards/services/authoringService.ts`: one unused export removed;
  fold-in addendum removes `AuthoringGoalRequest`/`AuthoringResult` from its import + re-export.
- `frontend/src/features/dashboards/types/authoring.ts`: fold-in addendum deletes
  `AuthoringGoalRequest`/`AuthoringResult` (zero remaining consumers).
