# connector-completion-token Specification
...
Evaluation-1.md CR5: the wire shapes this delta introduces are pinned as JSON Schema, alongside the
governing spec text rather than inferred solely from `ConnectorEntityProtocol.scala`:

- `schemas/sources/completion-request.schema.json` -- `POST /api/connectors/completion` body.
- `schemas/sources/completion-token-response.schema.json` -- `POST /api/connectors/pending` and
  `POST /api/connectors/:id/completion-token` response.
- `schemas/sources/pending-connector-auth-shape-response.schema.json` -- `GET
  /api/connectors/completion?token=...` response (the completion page's own lookup, added in the same
  cycle per evaluation-1.md CR4).
