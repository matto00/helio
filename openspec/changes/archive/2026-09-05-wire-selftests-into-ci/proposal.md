...
- `npm run check:openspec:selftest` is wired into `.github/workflows/ci.yml` alongside the existing self-tests.
...
- `openspec-spec-hygiene`: the hygiene guard's self-test SHALL run as a merge-blocking CI step, not pre-commit
  only.

## Impact

- `scripts/check-no-credential-in-agent-surface.selftest.mjs` — the terminal reporting branch.
- `.github/workflows/ci.yml` — one added `- run:` step plus an explanatory comment.
