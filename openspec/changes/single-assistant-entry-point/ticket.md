...
- [ ] `AuthoringGoalRequest`/`AuthoringResult` (`frontend/src/features/dashboards/types/
      authoring.ts`) are deleted, along with their re-export in `authoringService.ts`, once this
      ticket's own deletions leave them with zero remaining consumers (fold-in, added after initial
      delivery — see Context/Notes).
...
- **Post-delivery fold-in (PR #345 merged; addendum before Phase 4 cleanup):** the evaluator and the
  final-gate skeptic both independently flagged, as a non-blocking suggestion, that
  `AuthoringGoalRequest`/`AuthoringResult` (`frontend/src/features/dashboards/types/authoring.ts`)
  have zero real consumers frontend-wide once this ticket's own deletion of
  `useDashboardAuthoringStream.ts` — their only importer — landed. `authoringService.ts`'s own
  barrel `export type { ... }` line re-exports both but is itself never imported by anything for
  either name (confirmed via grep — no consumer anywhere outside `authoring.ts`/`authoringService.ts`
  themselves). The coordinator triaged this **fold-in** (not standalone) on the grounds that it is a
  trivial, zero-risk, directly-caused consequence of this exact ticket's own deletion — cheap enough
  that a separate ticket would be pure overhead. Scope: delete both interfaces from `authoring.ts`
  and drop them from `authoringService.ts`'s re-export line and import list. No other exports in
  either file are touched (`AuthoringErrorKind`, `AuthoringOutcome`, `AuthoringDisplayTurn`,
  `AuthoringConversationView`, `fetchAuthoringConversation`, `postAuthoringOutcome` all remain — all
  still have real consumers, confirmed via grep before this addendum was written).
