...
// observe the rendered menu-item height at a phone viewport. This test
// statically asserts the CSS source keeps the mobile-scoped ≥44px override for
// the shared actions-menu items (`.actions-menu__item`).
//
// The actions menu is a shared kebab menu reachable on phone via panel cards,
// the dashboard list, and the sidebar item list, so its item rows must meet the
// 44px tap-target convention (MobileNavSheet.css / PanelDetailModal.css's mobile
// block) at the mobile-shell breakpoint, without touching ActionsMenu.tsx logic
// or desktop density (the rule lives inside a `max-width: 768px` media block).

const CSS_PATH = path.join(__dirname, "ActionsMenu.css");
...
describe("ActionsMenu.css — mobile ≥44px tap targets (HEL-308)", () => {
  const mobileBlock = findMediaBlock(css, "max-width: 768px");

  it("the actions-menu items get min-height: 44px at the mobile-shell breakpoint", () => {
    const body = findRuleBody(mobileBlock, ".actions-menu__item");
    expect(body).toMatch(/min-height:\s*44px\s*;/);
  });

  it("the actions-menu items switch to flex centering so the label stays vertically centered", () => {
    const body = findRuleBody(mobileBlock, ".actions-menu__item");
