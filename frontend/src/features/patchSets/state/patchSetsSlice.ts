...
// ── Post-apply cache invalidation (skeptic evaluation-1.md CR1) ────────────
//
...
// always triggers a real request), patch-set apply MUTATES existing,
...
// data after a successful Accept (reproduced live in evaluation-1.md: the
// backend write succeeds, but the SPA doesn't reflect it without a manual
// reload). Scope: `panel`/`dashboard` edits only (the two kinds this ticket's
// own demo/fixture entry point can produce today, HEL-408 design.md D6);
// `dataType`/`dataSource`/`pipeline` edits have their own slices with the
// identical staleness exposure once a real patch-set producer can target
// them (no such caller exists yet — this ticket's own Non-Goal), and are
// left as a follow-on of the same shape.
 ...
 *  `unrecoverable` delete-rollback (HEL-406 `EditOutcome` doc).
 *
...
export function invalidateAffectedState(
  patchSet: PatchSet,
  response: PatchSetApplyResponse,
  dispatch: ThunkDispatch<RootState, unknown, UnknownAction>,
  getState: () => RootState,
): void {
  const touchedPanelDashboardIds = new Set<string>();

  response.edits.forEach((outcome: EditOutcome) => {
...
>("patchSets/applyPatchSet", async (patchSet, { dispatch, getState, rejectWithValue }) => {
  try {
    ...
    invalidateAffectedState(patchSet, response, dispatch, getState);
    return response;
  } catch (err) {
    const serverMessage =
      isAxiosError(err) && typeof err.response?.data?.message === "string"
