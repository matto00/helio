// Markdown panel's Content editor (HEL-909: literal-only — the Source/bound
// mode was stripped along with the retired DataType-field-mapping model per
// design.md's explicit resolution; a Markdown panel is dashboard-native and
// carries no Output).

import { forwardRef, useImperativeHandle, useState } from "react";

import { updatePanelMarkdownContent } from "../../state/panelsSlice";
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        save: async () => {
          ...
          try {
            await dispatch(updatePanelMarkdownContent({ panelId: panel.id, content })).unwrap();
            return { ok: true };
          } catch {
