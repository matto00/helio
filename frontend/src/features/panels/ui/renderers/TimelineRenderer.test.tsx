...
const FIELD_MAPPING = { time: "when", event: "what" };
...
describe("TimelineRenderer — chronological ordering (HEL-317)", () => {
  it("renders one entry per bound row, each with its own time and event text", () => {
    const { container } = render(
      <TimelineRenderer fieldMapping={FIELD_MAPPING} sort="asc" rawRows={ROWS} headers={HEADERS} />,
    );
...
describe("TimelineRenderer — degradation at the data edges", () => {
  it("shows a No data state when there are zero rows", () => {
    render(
      <TimelineRenderer fieldMapping={FIELD_MAPPING} sort="asc" rawRows={[]} headers={HEADERS} />,
    );
