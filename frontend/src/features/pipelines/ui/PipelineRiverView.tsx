...
export function PipelineRiverView({
  ...
  const primaryLane = laneGraph.lanes.find((l) => l.id === laneGraph.primaryLaneId);
  ...
  function handleCardDrop(e: DragEvent<HTMLDivElement>) {
    ...
    if (draggedIndex !== null && overIndex !== null && overIndex !== draggedIndex) {
      ...
      const primaryId = currentGraph.primaryLaneId;
      if (primaryId) {
  ...
  const handleMoveUp = useCallback(
    (stepId: string) => {
      ...
      const primaryId = currentGraph.primaryLaneId;
      if (!primaryId) return;
      const lane = currentGraph.lanes.find((l) => l.id === primaryId);
      const index = lane?.steps.findIndex((s) => s.id === stepId) ?? -1;
      if (index <= 0) return;
      onReorderSteps(reorderLane(currentGraph, primaryId, index, index - 1));
    },
    [onReorderSteps],
  );
  ...
  const handleMoveDown = useCallback(
    (stepId: string) => {
      ...
      const lane = currentGraph.lanes.find((l) => l.id === primaryId);
      const index = lane?.steps.findIndex((s) => s.id === stepId) ?? -1;
      if (index === -1 || !lane || index >= lane.steps.length - 1) return;
      onReorderSteps(reorderLane(currentGraph, primaryId, index, index + 1));
    },
    [onReorderSteps],
  );
