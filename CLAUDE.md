...
| `ANTHROPIC_API_KEY`                | Conditional | Anthropic API key for the server-side Claude client (`com.helio.ai`). Required only once a caller actually constructs a `ClaudeClient`/`HttpClaudeTransport` (no route consumes this yet — see HEL-390); sourced from Secret Manager (`helio-anthropic-api-key`) via `infra/deploy-backend.sh`'s `--set-secrets`, never set in `infra/.env.deploy.example`. Never committed, never logged. See HEL-390.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `RESEND_API_KEY`                   | Conditional | Bearer credential for the Resend REST API (`com.helio.email`), the backend's outbound-email capability. Required only for `POST /api/beta-access/request` to send its owner-notification email; unset degrades that one endpoint to `503` (`EmailUnconfigured`) — the backend still boots normally, and invite-code redemption never depends on this. Never logged; redacted from `EmailConfig.toString`. See HEL-704.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `HELIO_EMAIL_FROM`                 | Conditional | Sender address for outbound email (`com.helio.email.EmailConfig`). Required alongside `RESEND_API_KEY` — either missing degrades `POST /api/beta-access/request` to `503`. See HEL-704.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
