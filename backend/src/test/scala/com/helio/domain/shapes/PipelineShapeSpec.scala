...
class PipelineShapeSpec extends AnyWordSpec with Matchers {
  ...
  private val expectedIds: Set[String] = Set("passthrough", "single-row", "top-n", "time-series")
  ...
  "PipelineShape.Registry" should {
    ...
    "contain exactly the passthrough, single-row, top-n, and time-series shapes, keyed by their ids" in {
    ...
    "match the independently-authored id set, with no drift between Registry and the literal set" in {
      ...
      PipelineShape.Registry should have size 4
    }
  }
}
