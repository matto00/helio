...
final class PipelineRunHistoryRoutes(runService: PipelineRunService)(implicit ec: ExecutionContext)
  ...
  val routes: Route =
    pathPrefix("pipelines" / PipelineIdSegment / "run-history") { pipelineId =>
      pathEndOrSingleSlash {
        get {
          ServiceResponse.run(runService.history(pipelineId))(identity)
        }
      }
    }
}
