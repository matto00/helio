...
final class PipelineRunSubmitRoutes(runService: PipelineRunService)(implicit ec: ExecutionContext)
      ...
      pathEndOrSingleSlash {
        post {
          parameter("dry".?) { dryParam =>
            ...
            ServiceResponse.run(runService.submit(pipelineId, isDry)) { result =>
