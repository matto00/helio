...
(the MDC-propagating `ExecutionContext` wrapper), `ai/` (the outbound
Anthropic Claude client), `email/` (outbound transactional email via
Resend). See each subdirectory's own README for what belongs there.

No file lives directly in `infrastructure/` — every file is under one of
the six subdirectories above.
