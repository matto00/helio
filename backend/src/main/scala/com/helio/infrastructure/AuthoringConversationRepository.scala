...
import com.helio.api.protocols.{AuthoringConversationProtocol, AuthoringDisplayTurn, DashboardProposal}
 ...
 *  mirrors `MetricRepository`'s `withUserContext` + JSONB-column-mapping conventions exactly.
 *  `apiHistory` (the raw `Vector[ClaudeMessage]` shape needed to continue the Claude conversation)
 *  is read here but NEVER serialized to the wire — `findDisplayById` is the only read
 *  `DashboardAuthoringService.getConversation`/`GET /api/authoring/conversations/:id` is ever
 *  wired to (design.md D7). */
class AuthoringConversationRepository(ctx: DbContext)(implicit ec: ExecutionContext) {
  ...
  /** The display-only subset `GET /api/authoring/conversations/:id` needs (design.md D7) —
   *  `apiHistory` is read from the row but discarded before returning; it never crosses into the
   *  wire-facing `AuthoringConversationView`. */
  def findDisplayById(
      id: AuthoringConversationId,
      user: AuthenticatedUser
  ): Future[Option[(Vector[AuthoringDisplayTurn], Option[DashboardProposal])]] =
    findById(id, user).map(_.map(r => (r.displayTurns, r.latestProposal)))

  /** Appends a completed turn — updates `api_history`/`display_turns`/`latest_proposal`/
   *  `total_tokens_used` together, in the SAME row-scoped `withUserContext` transaction (design.md
   *  D3; mirrors `MetricRepository.update`'s update-then-reselect shape). Returns `None` for a
   *  missing/foreign-owned id — the caller (`DashboardAuthoringService`) already checked via
   *  `findById` before reaching here in every real call path, but this stays safe standalone. */
  def appendTurn(
      id: AuthoringConversationId,
      user: AuthenticatedUser,
    ...
    val action = table
      ...
      .map(r => (r.apiHistory, r.displayTurns, r.latestProposal, r.totalTokensUsed, r.updatedAt))
      .update((apiHistory, displayTurns, Some(latestProposal), totalTokensUsed, updatedAt))
...
object AuthoringConversationRepository {
   ...
   *  either destructures it into `DashboardAuthoringResponse`/`AuthoringStreamEvent.Result` (just
   *  the new `conversationId`) or into `AuthoringConversationView` (`displayTurns`/
   *  `latestProposal` only, via `findDisplayById`). */
  ...
  private[infrastructure] class AuthoringConversationTable(tag: Tag)
    ...
    def * =
      (id, ownerId, apiHistory, displayTurns, latestProposal, totalTokensUsed, createdAt, updatedAt)
