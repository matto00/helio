...
| 3   | `POST /api/pipelines/<profitagg-id>/run` returns 422 "DataSource not found for join" | The ProfitAgg join step has an empty config (`{}`); `secondaryInput` defaults to `Source("")` (HEL-911), which resolves to nothing       |
...
-- AC1: config should contain secondaryInput (HEL-911)
