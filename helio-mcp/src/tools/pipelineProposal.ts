...
import { boundPipelineStepSchema } from "./write.js";
...
export const pipelineProposalInputSchema = {
  ...
  // Reuses write.ts's boundPipelineStepSchema (hoisted + exported, HEL-385
  // design.md D3) instead of a second, drift-prone copy.
...
export function registerPipelineProposalTools(server: McpServer, api: HelioApi): void {
  server.registerTool(
    "propose_pipeline",
    {
      ...
      description:
        "Assemble a pipeline proposal (source + ordered transform steps + output DataType name) " +
        ...
        "the same ordered `{type, config}` list add_pipeline_step accepts (see that tool's " +
        "description for the full per-op config catalogue) — may be empty to bind the raw source " +
        "schema unchanged. Read-only-checks a given `sourceId` resolves among your data sources and " +
    ...
    ({ pipelineName, source, outputDataTypeName, steps }) =>
      guarded(() =>
  ...
  server.registerTool(
    "analyze_pipeline_proposal",
    {
      ...
      description:
        ...
        "propose_pipeline is not required first). Returns { sourceName, outputDataTypeName, " +
        "sourceSchema, steps } — `steps` is the same per-step " +
        ...
        "one (no id/outputDataTypeId exist yet, since nothing is persisted).",
      inputSchema: pipelineProposalInputSchema,
    },
    ({ pipelineName, source, outputDataTypeName, steps }) =>
      guarded(() =>
  ...
  server.registerTool(
    "apply_pipeline_proposal",
    {
      ...
      description:
        ...
        "pipeline summary/output DataType id/run result.",
      inputSchema: pipelineProposalInputSchema,
    },
    ({ pipelineName, source, outputDataTypeName, steps }) =>
      guarded(() =>
