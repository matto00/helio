 ...
 * `update_pipeline_step` tools (`write.ts`). Mirrors `metricSchemas.ts`
 * exactly (design.md D3, revised at design-gate round 1 — see that module's
 * own header comment for the full rationale): split into its own small
 * module, still `write.ts`'s exclusive, local concern, so a unit test can
 * import just this narrow surface without pulling `write.ts`'s full ~20-tool
 * Zod-schema surface into the compile graph (pathologically expensive to
 * type-check under this repo's root `tsconfig.json`/ts-jest combination —
 * see `write.test.ts`).
 */

import { z } from "zod";
import type { UpdateDataTypeRequest, UpdatePipelineStepRequest } from "../types.js";
